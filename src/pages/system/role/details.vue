...
const isNew = !route.query.roleName
...
async function getHomeMenuList() {
  ...
  menuList.value = res.data.body.resultList
  allMenuOperationList.value = res.data.body.resultList
  // 初始默认勾选项
  defaultCheckedKeys.value = res.data.body.menuIdList || []
}

async function submit() {
  // const params = { ...form }
  if (!isNew)
    userForm.parentId = route.query.roleId
  ...
  <div id="addRole" class="pageCommonStyle page-container text-sm bg-white p-3">
    ...
    <ElForm ref="userFormRef" class="demo-ruleForm" label-width="120px" :model="userForm" :rules="userFormRules">
      <ElFormItem label="角色名称" prop="roleName">
        <ElInput v-model="userForm.roleName" maxlength="10" placeholder="请选择角色名称" style="width:60%;" />
      </ElFormItem>
